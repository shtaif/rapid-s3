...
// Reusable middleware to make sure the "user" part in the request path is valid
...
const filesApiRouter = new KoaRouter({prefix: '/files'})
    // Route for file download / metadata:
    ...
    // Route for a updating a file record (only the `accessLevel` field is allowed to be modified)
    ...
    // Route for marking a file record as "deleted"
